			...
			<div class="inner">
				...
				<table class="table" style="width: 1000px; margin-left: auto; margin-right: auto;">
					<tr style="border-bottom: none;">
						<th colspan="3" style="padding-bottom: 0;">자유게시판</th> 
					</tr>
					...
					<!-- 메인글을 출력한다. -->
					<c:set var="list3" value="${freeboardList.list3}"/>						
					...
					<c:if test="${list3.size() != 0}">
						<c:forEach var="fo" items="${list3}">
							<tr>
								<td align="center">${fo.idx}</td>
								<td style="padding-left: 15px;">
									<!-- 제목에 태그 적용 불가 -->
									<c:set var="subject" value="${fn:replace(fo.subject, '<', '&lt;')}"/>
									<c:set var="subject" value="${fn:replace(subject, '>', '&gt;')}"/>
									<!-- 제목에 하이퍼링크를 걸어준다. -->
									<a href="freeboardIdx?idx=${fo.idx}&currentPage=${currentPage}&cnum=${fo.cnum}">
										${subject}
									</a>	
								</td>
								<!-- 오늘 글, 어제 글 다르게 표시 -->
						...
						<td colspan="5" align="center" style="padding-bottom: 0;">
							...
							<c:if test="${freeboardList.startPage > 1}">
								<button 
									...
									onclick="location.href='?cnum=${mvo.cnum}&currentPage=${freeboardList.startPage - 1}'"
								>이전</button>
							</c:if>
							<c:if test="${freeboardList.startPage <= 1}">
								<button 
							...
							<c:if test="${freeboardList.endPage < freeboardList.totalPage}">
								<button 
									...
									onclick="location.href='?cnum=${mvo.cnum}&currentPage=${freeboardList.endPage + 1}'"
								>다음</button>
							</c:if>
							<c:if test="${freeboardList.endPage >= freeboardList.totalPage}">
								<button 
							...
							<c:if test="${freeboardList.currentPage < freeboardList.totalPage}">
								<button
									...
									onclick="location.href='?cnum=${mvo.cnum}&currentPage=${freeboardList.totalPage}'"
								>마지막</button>
							</c:if>
							<c:if test="${freeboardList.currentPage >= freeboardList.totalPage}">
								<button 
					...
					<tr class="actions" style="border-bottom: none; border-top: none;">
						<td colspan="4" align="right" style="padding-top: 0;">
							<input class="button big" type="button" value="글쓰기"
								onclick="location.href='freeboardInsert?currentPage=${currentPage}'">					
						</td>
					</tr>
				</table>
			</div> <!-- <div class="inner"> -->
