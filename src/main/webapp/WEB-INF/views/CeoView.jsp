	...
	<div id="wrapper">
		<div id="main">
			<div class="inner">
				...
				<table class="table" style="width: 1280px; margin-left: auto; margin-right: auto;">
					<tr style="border-bottom: none;">
						<th colspan="7" style="padding-bottom: 0;">결재 목록</th>
					</tr>
					<tr style="border-top: none;">
						<td colspan="7" align="right" style="font-size: 13px; padding-top: 0;">
					...
					<tr class="tr">
						...
						<td style="width: 150px; text-align: center;">결재 종류</td>
					...
					<c:if test="${list.size() != 0}">
						<c:forEach var="co" items="${list}">
							<tr>
								...
								<td align="center">${co.paper}</td>
								<td style="padding-left: 30px;">
									<!-- 제목에 태그를 적용할 수 없게 한다. -->
									<c:set var="subject" value="${fn:replace(co.subject, '<', '&lt;')}"/>
									<c:set var="subject" value="${fn:replace(subject, '>', '&gt;')}"/>
									<!-- 제목에 하이퍼링크를 걸어준다. -->
									<a href="CeoComment?idx=${co.idx}&cnum=${co.cnum}&currentPage=${currentPage}&reason=${co.reason}&paper=${co.paper}&approval=${approval}">
										${subject}
									</a>
								</td>
								<td align="center">
						...
						<td colspan="7" align="center">
							...
							<c:if test="${ceoList.startPage > 1}">
								<button 
									...
									onclick="location.href='?cnum=${mvo.cnum}&currentPage=${ceoList.startPage - 1}'"
								>이전</button>
							</c:if>
							<c:if test="${ceoList.startPage <= 1}">
								<button 
							...
							<c:if test="${ceoList.endPage < ceoList.totalPage}">
								<button 
									...
									onclick="location.href='?cnum=${mvo.cnum}&currentPage=${ceoList.endPage + 1}'"
								>다음</button>
							</c:if>
							<c:if test="${ceoList.endPage >= ceoList.totalPage}">
								<button 
							...
							<button
								...
								onclick="location.href='?cnum=${mvo.cnum}&currentPage=${ceoList.totalPage}'"
							>마지막</button>
							</c:if>
							<c:if test="${ceoList.currentPage >= ceoList.totalPage}">
								<button 
					...
					<tr style="border-bottom: none; border-top: none;">
						<td colspan="7" align="center" style="padding: 0;">
							<form action="Ceo" method="post" style="margin-bottom: 0;">
					...
					<tr class="actions" style="border-bottom: none; border-top: none;">
						<td colspan="7" align="right" style="padding-top: 0;">
							<input class="button big" type="button" value="글쓰기"
								onclick="location.href='approvalInsert?currentPage=${currentPage}'">
						</td>
					</tr>
				</table>
			</div> <!-- <div class="inner"> -->
