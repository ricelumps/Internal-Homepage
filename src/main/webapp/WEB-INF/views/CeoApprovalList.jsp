...
<style type="text/css">
	...
	input[type=text] { /* 검색 내용 입력 */
		font-size: 13px;
		display: inline-block;
		width: 250px; 
		height: 30px; 
		border: none;
		border-bottom: 1.5px solid #7f888f;
		border-radius: 0;
		padding: 5px;
	}
	input[type=submit] { /* 검색 버튼 */
		font-size: 13px;
		font-weight: normal;
		width: 50px;
		height: 25px;
	...
	<div id="wrapper">
		<div id="main">
			<div class="inner">
				...
				<table class="table" style="width: 1180px; margin-left: auto; margin-right: auto;">
					<tr style="border-bottom: none;">
						<th colspan="6" style="padding-bottom: 0;">[${paper}] 결재 목록</th>
					</tr>
					<tr style="border-top: none;">
						<td colspan="6" align="right" style="font-size: 13px; padding-top: 0;">
					...
					<c:if test="${list.size() != 0}">
						<c:forEach var="co" items="${list}">
							<tr>
								<td align="center" style="font-size: 13px;">${co.idx}</td>
								<td align="center">${co.cnum}</td>
								<td style="padding-left: 30px;">
									<c:set var="subject" value="${fn:replace(co.subject, '<', '&lt;')}"/>
									<c:set var="subject" value="${fn:replace(subject, '>', '&gt;')}"/>
									<a href="CeoComment?idx=${co.idx}&cnum=${mvo.cnum}&currentPage=${currentPage}&reason=${co.reason}&paper=${co.paper}&approval=${approval}">
										${subject}
									</a>
								</td>
								<td align="center">
								...
								<td align="center" style="font-size: 13px;">
									<c:if test="${date.year == co.writeDate.year && date.month == co.writeDate.month &&
										...
										TODAY <fmt:formatDate value="${co.writeDate}" pattern="HH:mm"/>
									</c:if>			
									<c:if test="${date.year != co.writeDate.year || date.month != co.writeDate.month ||
										date.date != co.writeDate.date}">
						...
						<td colspan="6" align="center">
							...
							<c:if test="${ceoList.currentPage > 1}">
								<button 
									...
									type="button" 
									title="첫 페이지로 이동합니다." 
									onclick="location.href='?cnum=${mvo.cnum}&currentPage=1&paper=${paper}'"
								>처음</button>
							</c:if>
							<c:if test="${ceoList.currentPage <= 1}">
								<button 
							...
							<c:if test="${ceoList.startPage > 1}">
								<button 
									...
									onclick="location.href='?cnum=${mvo.cnum}&currentPage=${ceoList.startPage - 1}&paper=${paper}'"
								>이전</button>
							</c:if>
							<c:if test="${ceoList.startPage <= 1}">
								<button 
							...
							<c:forEach var="i" begin="${ceoList.startPage}" end="${ceoList.endPage}" step="1">
								...
								<c:if test="${ceoList.currentPage != i}">
									<button 
										...
										type='button' 
										title="${i}페이지로 이동합니다."
										onclick="location.href='?cnum=${mvo.cnum}&currentPage=${i}&paper=${paper}'"
									>${i}</button>
								</c:if>
							</c:forEach>
							...
							<c:if test="${ceoList.endPage < ceoList.totalPage}">
								<button 
									...
									onclick="location.href='?cnum=${mvo.cnum}&currentPage=${ceoList.endPage + 1}&paper=${paper}'"
								>다음</button>
							</c:if>
							<c:if test="${ceoList.endPage >= ceoList.totalPage}">
								<button 
							...
							<button
								...
								onclick="location.href='?cnum=${mvo.cnum}&currentPage=${ceoList.totalPage}&paper=${paper}'"
							>마지막</button>
							</c:if>
							<c:if test="${ceoList.currentPage >= ceoList.totalPage}">
								<button 
					...
					<tr style="border-bottom: none; border-top: none;">
						<td colspan="7" align="center" style="padding: 0;">
							<form action="approvalList" method="post" style="margin-bottom: 0;">
					...
					<tr class="actions" style="border-bottom: none; border-top: none;">
						<td colspan="6" align="right" style="padding-top: 0;">
							<input class="button big" type="button" value="글쓰기"
								onclick="location.href='approvalInsert?paper=${paper}&currentPage=${currentPage}'">
						</td>
					</tr>
				</table>
			</div> <!-- <div class="inner"> -->
